#!/usr/bin/env python3
"""
UnivFD (CVPR 2023) 复现脚本 —— 在本项目测试集上评估。

用法:
    python test_univfd.py

默认复用 ./data/test 下的测试数据（与 test_thesis.py 同一测试集）。
UnivFD 权重: ./UniversalFakeDetect/pretrained_weights/fc_weights.pth
...
UNIVFD_DIR = SCRIPT_DIR / "UniversalFakeDetect"
IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".bmp", ".webp"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="UnivFD baseline 测试")
    ...
    parser.add_argument("--arch", type=str, default="CLIP:ViT-L/14", help="模型架构")
    parser.add_argument("--ckpt", type=str,
                        default=r"C:\hejulian\fc_weights.pth",
                        help="UnivFD 权重路径")
    parser.add_argument("--batch_size", type=int, default=64, help="推理批大小")
    parser.add_argument("--output_dir", type=str, default=r"C:\hejulian\exp\univfd_baseline",
                        help="结果输出目录")
...
def load_univfd_model(arch: str, ckpt_path: str, device: torch.device) -> nn.Module:
    """加载 UnivFD 模型。"""
    ...
    model.eval()
    model.to(device)
    sys.path.remove(str(UNIVFD_DIR))
    print(f"[UnivFD] 模型加载完成: arch={arch}")
    return model
...
def main():
    ...
    if args.recalc:
        recalc_metrics_from_csv(Path(args.output_dir), args.threshold, "UnivFD (CLIP:ViT-L/14)")
        return

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[设备] {device}")

    model = load_univfd_model(args.arch, args.ckpt, device)
